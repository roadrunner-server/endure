...
//	c, err := endure2.NewContainer(nil, endure.SetLogLevel(level))
...
func TestEndure_VisualizeFile(t *testing.T) {
	...
	//`
	//	filename := "graph.txt"
	//	c, err := endure2.NewContainer(nil, endure.Visualize(endure.File, filename), endure.SetLogLevel(endure.ErrorLevel))
	//	assert.NoError(t, err)
	//
	//	assert.NoError(t, c.Register(&plugin4.S4{}))
	//	assert.NoError(t, c.Register(&plugin2.S2{}))
